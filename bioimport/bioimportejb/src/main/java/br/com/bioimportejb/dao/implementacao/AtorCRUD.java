...
public class AtorCRUD extends CRUDGenerico<Ator, Long> implements AtorDAO, Serializable{
	...
	public String buscarEmailPorLogin(String login) throws ExcecaoGenerica{
		...
		StringBuffer hql = new StringBuffer("select c.email FROM Ator c where c.email = :login ");
	...
	public boolean verificarExistenciaLogin(String login, Long id) throws ExcecaoGenerica{
		...
		StringBuffer hql = new StringBuffer("select count(c.id) FROM Ator c where c.email = :login ");
		if(null != id){
	...
	public boolean validarAtorPorLoginECodigo(String login, String codigo) {
		StringBuffer hql = new StringBuffer("select count(c.id) FROM Ator c where c.email = :login and c.codigo = :codigo and c.dataHoraValidadeCodigo > :agora");
	...
	public boolean validarAtorPorLoginESenha(String login, String senhaAtual) {
		StringBuffer hql = new StringBuffer("select count(c.id) FROM Ator c where c.email = :login and c.senha = :senhaAtual");
	...
	public List<String> buscarPorPerfil(PerfilEnum ad) throws ExcecaoGenerica {
		try{
			...
			hql.append("select ator.email from Ator ator left outer join ator.perfils as perfil ");
